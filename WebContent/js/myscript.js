...
$(document)
		.ready(
				function() {
					/* Get All Control */
					var keyword = $('#txt-search'), className = $('#class-name'), studentInfo = $('div#student-info'), txtId = $('#txt-id'), txtName = $('#txt-name'), cboGender = $('#cbo-gender'), cboUniversity = $('#cbo-university'), cboClass = $('#cbo-class'), btnAdd = $('#add-new-student'), alertId = $('#alert-id'), validateId = $('#validate-id'), alertName = $('#alert-name'), validateName = $('#validate-name');
					...
					function updateStudent(td) {
						$
								.ajax({
									...
									success : function(data) {
										if (data == "success") {
											// Because if cbo.text = sum of
											// option's text
											var gender = (cboGender.val() == 1) ? "Male"
													: "Female";
					...
					function clear() {
						alertId.hide();
						alertName.hide();
						$('#validate-id, #validate-name').removeClass(
								'has-error');
